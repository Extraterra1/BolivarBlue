  ...
  return (
    <Card className={`min-w-[280px] md:min-w-[300px] rounded-xl ${cardClasses} hover:shadow-2xl transition-all duration-300 cursor-pointer active:scale-95`}>
      <CardHeader className="flex gap-3 px-5 md:px-6 pt-5 md:pt-6 pb-3 md:pb-4">
        <div className={`w-11 h-11 md:w-12 md:h-12 flex items-center justify-center rounded-xl overflow-hidden border ${getAccentColor()}`}>
          {logo ? (
            <img
              src={logo}
              alt={title}
              className={`w-8 h-8 md:w-9 md:h-9 object-contain ${logo?.toLowerCase().includes('bcv') && !isDark ? 'brightness-0' : ''}`}
            />
          ) : (
